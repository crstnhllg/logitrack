...
def test_login_for_access_token(test_user):
    request_data = {"username": test_user.username, "password": "12345"}
...
def test_invalid_login():
    request_data = {"username": "invalid_username", "password": "invalid_password"}
    response = client.post("/auth/token", data=request_data)
...
def test_create_access_token(test_user):
    token = create_access_token(test_user.username, test_user.id, timedelta(minutes=60))
...
def test_get_current_user(db_session, test_user):
    ...
    payload = {"sub": test_user.username, "id": test_user.id, "exp": expires}
