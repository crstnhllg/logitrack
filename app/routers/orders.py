...
def get_all_orders(
    ...
    filters = {"destination": destination, "size": size, "status": order_status}
...
def get_order_by_id(
    db: db_dependency, user: user_dependency, order_id: int = Path(gt=0)
):
...
def add_order(db: db_dependency, user: user_dependency, order_request: OrderRequest):
    ...
    if order_request.vehicle_id is not None:
        assigned_vehicle = (
            db.query(Vehicle).filter(Vehicle.id == order_request.vehicle_id).first()
        )
...
@router.put(
    "/{order_id}/status", status_code=status.HTTP_200_OK, response_model=OrderResponse
)
