...
@router.get(
    "/{vehicle_id}", status_code=status.HTTP_200_OK, response_model=VehicleResponse
)
def get_vehicle_by_id(
    db: db_dependency, user: user_dependency, vehicle_id: int = Path(gt=0)
):
...
def add_vehicle(
    db: db_dependency, user: user_dependency, vehicle_request: VehicleRequest
):
    ...
    assigned_driver = (
        db.query(User)
        .filter(User.id == vehicle_request.driver_id, User.role == "driver")
        .first()
    )
    ...
    if (
        db.query(Vehicle)
        .filter(Vehicle.license_plate == vehicle_request.license_plate)
        .first()
    ):
        raise HTTPException(
...
def change_vehicle_driver(
    db: db_dependency,
    user: user_dependency,
    driver_request: VehicleDriverRequest,
    vehicle_id: int = Path(gt=0),
):
    ...
    new_driver = (
        db.query(User)
        .filter(User.id == driver_request.driver_id, User.role == "driver")
        .first()
    )
